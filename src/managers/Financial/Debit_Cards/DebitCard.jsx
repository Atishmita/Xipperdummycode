...
const DebitCard = ({ debitCard }) => {
  ...
  return (
    <div className="p-3 w-96 border rounded-lg shadow-sm bg-[#6D38C3] text-white">
      ...
      {showCard &&
      <p className="text-3xl pt-6">{debitCard.cardNumber}</p>}
      <p className="pt-3">{debitCard.name}</p>
      <div className="pt-4 flex items-center justify-between">
        <p>Expiry - {debitCard.expiry}</p>
        {console.log(debitCard.cardType)}
        {debitCard.cardType === "VISA" ? <img src={visaImg} alt="visa" className="h-12 rounded-full mr-2" /> : null}
        {debitCard.cardType === "MASTERCARD" ? <img src={masterCardImg} alt="visa" className="h-12 rounded-full mr-4" /> : null}
